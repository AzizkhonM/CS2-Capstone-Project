...
##### 3.2.1 ⚠️ Critical: `winner_*`/`loser_*` naming is leakage-prone

Columns like `winner_past3`, `winner_head2head_percentage`, `winner_mirage`, etc. are named **relative to the actual match winner** — meaning the column itself already encodes who won. These CANNOT be used directly as model features (this would be trivial leakage). Before training, every `winner_X`/`loser_X` pair must be **reframed to `team1_X`/`team2_X`** based on which team (team1 or team2) was the actual winner in that row. This was implemented in `04_model_training.ipynb` (`reframe_winner_loser()` function). Verified in the notebook — 12 such column pairs were reframed.
...
> **Note:** `avg_ADR` showed 2 unique values for one team (G2: 79.58 in 108/109 rows, 79.64 in 1 row) — a negligible rounding-level deviation (0.06), not affecting the STATIC classification or exclusion decision.
...
> **Additional note (verified in `01_eda.ipynb`):** Null rates increase with player index (`player_1` ≈0.1% → `player_5` ≈13%), consistent with some teams fielding only 4 players (missing stand-in/5th player data). Since this entire column group is already excluded from modeling as STATIC, these nulls have no impact on the feature pipeline.
...
> **Additional note (verified in `01_eda.ipynb`):** Same null pattern as Team1 — `team2_player_5_*` ≈13% null, `team2_player_4_*` ≈3% null. No impact on modeling, as this group is fully excluded.
...
> **Note (found in `04_model_training.ipynb`):** `team1/2_totalwinrate` and `team1/2_totallossrate` are perfectly collinear (`totallossrate = 1 - totalwinrate`), producing mirrored, unstable coefficients when both are used together in Logistic Regression. `totallossrate_diff` was dropped from the final feature set for this reason.
...
### 6.5 Elo validation results (`03_baseline_elo.ipynb`)
